//
//  ShazamAudioEqualizerView.swift
//  DynamicNotch
//
...
struct ShazamAudioEqualizerView: NSViewRepresentable {
    ...
    init(
        ...
        bandLevels: [Float] = [0.18, 0.18, 0.18, 0.18, 0.18],
        colors: [NSColor] = [.cyan, .blue],
    ...
    func makeNSView(context: Context) -> ShazamAudioEqualizerNSView {
        let view = ShazamAudioEqualizerNSView()
        view.setBarHeight(barHeight)
        view.setBarWidth(barWidth)
        view.setColors(colors)
        view.setListening(isListening)
        view.setBandLevels(bandLevels.map { CGFloat($0) })
        return view
    }

    func updateNSView(_ nsView: ShazamAudioEqualizerNSView, context: Context) {
        nsView.setBarHeight(barHeight)
        nsView.setBarWidth(barWidth)
        nsView.setColors(colors)
        nsView.setListening(isListening)
        nsView.setBandLevels(bandLevels.map { CGFloat($0) })
    }

    static func dismantleNSView(_ nsView: ShazamAudioEqualizerNSView, coordinator: ()) {
        nsView.stop()
    }
}

final class ShazamAudioEqualizerNSView: NSView {
    private enum Metrics {
        static let barCount = 5
        static let defaultBarWidth: CGFloat = 2.5
        static let barSpacing: CGFloat = 2.5
        static let defaultHeight: CGFloat = 16
        static let minimumScale: CGFloat = 0.18
        static let animationDuration: TimeInterval = 0.10
        static let timerInterval: TimeInterval = 0.08
    }
    ...
    private var bandLevels: [CGFloat] = [0.18, 0.18, 0.18, 0.18, 0.18]
    private var barColors: [NSColor] = []
    ...
    func setBandLevels(_ levels: [CGFloat]) {
        guard levels.count == Metrics.barCount else { return }
    ...
    func setColors(_ colors: [NSColor]) {
        ...
        if colors.isEmpty {
            cgColors = [NSColor.cyan.cgColor, NSColor.blue.cgColor]
...
private extension ShazamAudioEqualizerNSView {
    func setup() {
        wantsLayer = true
        ...
        gradientLayer.colors = [NSColor.cyan.cgColor, NSColor.blue.cgColor]
    ...
    func animateBars() {
        ...
        for (index, barLayer) in barLayers.enumerated() {
            ...
            let bandLevel = index < bandLevels.count ? bandLevels[index] : Metrics.minimumScale
            let targetScale = min(max(bandLevel, Metrics.minimumScale), 1.0)
